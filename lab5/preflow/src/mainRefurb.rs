	...
	for _ in 0..NUMBER_OF_THREADS {
		...
		let h = thread::spawn(move || {
			...
			while !excess_internal.lock().unwrap().is_empty() {
				...
				let mut unode = node_array[u].lock().unwrap();
				//pr!("u = {}",&unode.i);
				let mut vnode = Node::new(0);
				...
				for e in iter{
					...
					let vnode = node_array[v].lock().unwrap();
					pr!("2");
					...
					let mut h1 = unode.h;
					pr!("3");
					let mut h2 = /*node_array[v].lock().unwrap().h;*/ vnode.h;
					pr!("4");
					if h1 > h2 && (b*f < cap) {
						break;
					}else{
				...
				if v != n {
							...
							push(&mut unode,&mut vnode,&mut edge_array[edge_index].lock().unwrap(),&mut excess_internal.lock().unwrap(),&n);
							pr!("pushing done");
							drop(unode);
							drop(vnode);
						...
						pr!("relabling");
						relabel(&mut excess_internal.lock().unwrap(), &n,&mut unode);
						pr!("Done relabling");
						drop(unode);
						drop(vnode);
					}
