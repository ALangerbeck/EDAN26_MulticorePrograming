...
static void enter_excess(graph_t* g, node_t* v)
{
	...
	if (v != g->t && v != g->s) {
		...
		g->excess = v;
		g->numberInExcess += 1;
		pr("Excess contains %d nodes after entering node %d\n",g->numberInExcess,id(g,v));
	}
...
static node_t* leave_excess(graph_t* g)
{
	...
	if (v != NULL){
		g->excess = v->next;
		g->numberInExcess -=1;
		pr("Excess contains %d nodes after leaving node %d\n",g->numberInExcess,id(g,v));
	}
...
static void distributeNodes(graph_t* g,arg_t* threadlist){
	...
	while(((u = leave_excess(g)) != NULL)){
			...
			assert(g->numberInExcess > (-2));
			//pr("Number of nodes left in excess: %d\n",g->numberInExcess);
		...
		for (int j = 0; j < intNumOfinstuct; j++)
		{
			...
			if ((tempInstruct->relableOrPush) == 0)
			{
				...
				//push(g,tempInstruct->u,tempInstruct->v,tempInstruct->edge ,tempInstruct->amount);
					if (tempInstruct->u->e > 0)
					{
						enter_excess(g,tempInstruct->u);			
					}


					if ((atomic_load((&tempInstruct->v->futureExcessChange))) != 0)
					{	
